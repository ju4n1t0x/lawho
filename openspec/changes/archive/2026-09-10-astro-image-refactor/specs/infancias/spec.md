...
(Previously: spec referenced `comunidad-abuela.jpg` and rendered via raw `<img src={abuela.src}>`)
...
(Previously: spec referenced `ninos-1.jpg`, `ninos-2.jpg`, `mision-1.jpg` and rendered via raw `<img src={item.img.src}>`)
