...
1. **Stack mínimo**: Astro 7.2 y solo biblioteca estándar; ninguna librería ni framework adicional, salvo para tests y las siguientes excepciones:
   - TailwindCSS v4 (junto con su plugin de Vite `@tailwindcss/vite`) — estilos.
   ...
   - `sharp` — motor nativo de imágenes requerido por el pipeline `astro:assets` (`<Image>`) en build; pnpm no expone la optionalDependency de Astro a nivel proyecto.
